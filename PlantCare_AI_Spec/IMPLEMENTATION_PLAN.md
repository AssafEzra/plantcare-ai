...
| ~~A15~~ | No E2E tooling named for Streamlit | **REVISED in PR 31 — the original answer was wrong and it cost six defects.** httpx + `AppTest` test the two halves separately: `AppTest` stubs `api_client`, the journeys render nothing. Every serious bug in this build lived in the seam between them, where neither layer looks. Playwright now drives the real interface against the real API and a *live* model, marked `browser` and excluded from CI | — |
...
| PR 28 | The identification confirmation screen lived only in the Add Plant wizard's `st.session_state`, so it belonged to the browser tab that started the flow. A user closed the tab between the analysis and the question and was left with two plants **identified in the database and permanently unidentified in the interface** — the plant's own page said "הצמח עדיין לא זוהה" and offered nothing to press, and no endpoint exposed a plant's identifications at all. They reported it as "it failed to identify the plant" | The user, adding a plant. Every test passed: the wizard's own path works, and nine journeys drive it in one uninterrupted sequence — which is precisely the assumption the defect lives outside of |
| PR 28 | `plants.name` was never written. A2 settled in PR 2 that the name is set at confirmation, `API_CONTRACTS` wrote the fallback rule in PR 13, and neither the payload nor the workflow ever carried it — so every plant in the product was called "ללא שם" | Reading the two plants above out of DEV. The card renders `name or 'ללא שם'` and was doing exactly what it was told |
| PR 29 | One `AI_REQUEST_TIMEOUT_SECONDS=90` shared by four agents. Identification fits in it at ~30s; Knowledge research — thirteen Hebrew prose sections against `max_tokens=8000` with source retrieval — does not. The **first real research run** died at 90,354 ms with `AGENT_TIMEOUT`, which is deliberately not retried, so the draft went FAILED, the plant stayed `KNOWLEDGE_PENDING` and there was nothing in the admin queue to approve | The user, asking why no knowledge draft was created. Unreachable by the suite by construction: a mock provider answers instantly, so every timeout test proves the gateway handles one, never that the budget is right — and nothing anywhere looked at *how* the request was made |
| PR 30 | PR 29's switch to streaming moved where schema validation happens. `parse()` returned a message the provider validated; the streaming helper validates during accumulation and raises pydantic's `ValidationError` out of the iterator, which is not one of the provider's error types — so it escaped the gateway entirely: **no retry, no execution row, a flat AGENT_FAILED**. §23's retry budget and §25's visible failure were switched off for all four agents by a change that looked like transport | A user's health check doing nothing. Surfaced by a model returning `priority: 6` against a `le=5` bound — a fair reading of an unlabelled 1-5 field as "the sixth recommendation", which is now described in the schema |
| PR 30 | Only the Add Plant wizard polled its 202. The health check and both care-proposal routes fired theirs, said "the results will appear here in a moment", and never looked again — a failed run said nothing, a successful one appeared only on a manual reload | The user: "did a health check and nothing happened. didnt get result or status update." Two defects behind one sentence, and the second would have hidden the first indefinitely |
| PR 30 | `PUT /v1/plants/{id}/environment` shipped in PR 11 and no screen ever called it. תנאי הגידול rendered what was stored, said "עדיין לא הוגדרו" when nothing was, and offered no way to change that — while its own caption promised that changing the conditions triggers a care-plan review | The user: "i couldnt add or edit תנאי הגידול." The fourth instance of the same shape in six PRs: an endpoint and a screen, both correct, never joined |
| PR 31 | A static seam audit — every route against every path literal in the interface, every response field against what any screen reads — found **eight capabilities built, tested, ticked in `PROGRESS` and reachable by nobody**: rename a plant, delete a photograph, complete a task from the plant's own page, see where knowledge came from, disagree with an identification, and three admin screens (audit log, notification deliveries, agent requests) | Writing the audit after a user said "every feature I tested was full of bugs". Two of the eight had `PROGRESS` checkboxes whose own text was false — "Delivery logging: **user-visible**" was visible to nobody |
| PR 31 | The first browser run uploaded a flat green rectangle and identification answered "we could not identify this" — correctly. The second picked the *newest* image in DEV, which is always one of this suite's own solid-colour test uploads | The harness, not the product. Recorded because it is the trap any live UI suite walks into: a fixture that looks like data but cannot exercise the thing under test. It now picks by file size, since a photograph dwarfs a flat JPEG |
| PR 31 | A3 was implemented on **one** of the two roads into ACTIVE. The publication fan-out queued the INITIAL_PLAN proposal; confirming a species whose knowledge was already published queued nothing, so the plant went ACTIVE with no plan, no schedule and no reminders — and the only route to one was noticing an empty state and pressing a button | The first browser walk of the whole product. The integration test asserts the plant is ACTIVE and the journey asserts the same; both were right. Nobody asked what the user had *afterwards* |
| PR 31 | Agent work runs in `BackgroundTasks` inside the API process, so any restart kills every run in flight — and **nothing ever closed the request row**. It stays PROCESSING forever, the client polls, gives up politely, and says "still running" on every visit about a run that ended hours ago | A research request found sitting in PROCESSING for three hours across several `--reload` restarts while this PR was being written. A deploy does the same thing to production, which is what makes it a defect rather than a development annoyance |

The pattern: **mocks confirm the shape you assumed.** Thirty of these thirty-five were only
